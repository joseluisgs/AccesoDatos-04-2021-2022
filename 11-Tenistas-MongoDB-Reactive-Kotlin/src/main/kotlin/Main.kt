...
suspend fun limpiarDatos() = withContext(Dispatchers.IO) {
    ...
    if (MongoDbManager.database.getCollection<Tenista>().estimatedDocumentCount().awaitFirst() > 0) {
        logger.debug { "Borrando datos de la base de datos" }
        MongoDbManager.database.getCollection<Tenista>().drop().collect { println("Borrado") }
    }
}
